...
export async function POST(req: NextRequest) {
  ...
  try {
    const { month, entries, existingProfiles } = await req.json()
    ...
    const systemPrompt = `Ты — опытный психолог. Твоя задача — создать точные "заметки психолога" на основе дневниковых записей человека по имени Илья.

Эти заметки будут использоваться как контекст для AI-ассистента, который помогает Илье в повседневной жизни. Чем точнее и полезнее заметки — тем лучше ассистент сможет ему помочь.
...
СТИЛЬ:
- Пиши от третьего лица ("Илья склонен...", "Он замечает...")
- Конкретно и честно. Без лести, без осуждения.
- Используй прямые наблюдения из записей, а не абстрактные выводы.
- Это заметки для специалиста, не для самого Ильи — пиши честно о слабых местах.`
